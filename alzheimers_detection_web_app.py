...
with st.sidebar:
    ...
    if is_streamlit_cloud:
        ...
        **💡 Note:** Your files are securely stored in this session. If you refresh the page, you'll need to upload them again.
        """)
        st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="info-box">', unsafe_allow_html=True)
    ...
    uploaded_alz = st.file_uploader("Upload ALZ_Variant Data (preprocessed_alz_data.npz)", type=['npz'], key='alz_uploader')
    ...
    uploaded_mri_train = st.file_uploader("Upload MRI Training Data (train.parquet)", type=['parquet'], key='mri_train_uploader')
    ...
    uploaded_mri_test = st.file_uploader("Upload MRI Test Data (test.parquet)", type=['parquet'], key='mri_test_uploader')
