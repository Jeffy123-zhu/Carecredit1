...
if run_analysis:
    ...
    try:
        ...
        if not os.path.exists(BASE_DATASET_PATH):
            path_errors.append(f"数据集根目录不存在: {BASE_DATASET_PATH}")
        ...
        if use_alz_variant:
            ...
            if os.path.exists(alz_npz_path):
                try:
                    ...
                    st.success(f"✓ ALZ_Variant 数据加载成功: {X_train_alz.shape[0]} 样本")
                except Exception as e:
                    path_errors.append(f"ALZ_Variant 数据加载失败: {str(e)}")
            else:
                path_errors.append(f"ALZ_Variant 文件不存在: {alz_npz_path}")
            ...
            if os.path.exists(mri_train_path) and os.path.exists(mri_test_path):
                try:
                    ...
                    if combine_datasets and X_train_final is not None:
                        ...
                        st.success(f"✓ MRI 数据加载成功: {mri_train.shape[0]} 样本")
                    ...
                    path_errors.append(f"MRI 数据加载失败: {str(e)}")
            else:
                if not os.path.exists(mri_train_path):
                    path_errors.append(f"MRI 训练文件不存在: {mri_train_path}")
                if not os.path.exists(mri_test_path):
                    path_errors.append(f"MRI 测试文件不存在: {mri_test_path}")
        ...
        if X_train_final is None:
            ...
            st.markdown("### 💡 解决方案：")
            st.info(f"""
            1. **检查数据集路径**：当前路径为 `{BASE_DATASET_PATH}`
            
            2. **确保文件存在**：
               - ALZ_Variant: `{ALZ_VARIANT_PATH}/preprocessed_alz_data.npz`
               - MRI: `{MRI_PATH}/train.parquet` 和 `{MRI_PATH}/test.parquet`
            
            3. **在侧边栏修改路径**：如果数据集在其他位置，请在左侧"数据集路径"中修改
            
            4. **检查数据源选择**：确保至少勾选一个数据源（ALZ_Variant 或 MRI）
            """)
            st.stop()
